...
const expectedMetadata = (activationDigest, compatibilityDigest) => ({ "schema/id": JSON.stringify(SQLITE_SCHEMA_ID), "schema/version": "2", "schema/state": JSON.stringify("ready"), "schema/digest": JSON.stringify(SQLITE_SCHEMA_DIGEST), "compatibility/digest": JSON.stringify(compatibilityDigest), "activation/digest": JSON.stringify(activationDigest) });
const rowArray = (storage, sql, ...values) => storage.sql.exec(sql, ...values).toArray();
export class SqliteStore {
    ...
    initialize(activationDigest, compatibilityDigest) {
        if (!/^sha256:[0-9a-f]{64}$/.test(activationDigest) || !/^sha256:[0-9a-f]{64}$/.test(compatibilityDigest))
            throw new IncompatibleSqliteSchema("activation identity rejected");
        this.storage.transactionSync(() => {
            ...
            if (objects.length === 0) {
                ...
                for (const [key, value] of Object.entries(expectedMetadata(activationDigest, compatibilityDigest)))
                    this.storage.sql.exec("INSERT INTO meta(key,value_json) VALUES(?,?)", key, value);
    ...
    assertCompatible(activationDigest, compatibilityDigest) {
        ...
        if (JSON.stringify(tables) !== JSON.stringify(expected) || objects.some((row) => row.type !== "table"))
            throw new IncompatibleSqliteSchema("application schema rejected");
        for (const table of ["kv", "meta"]) {
            ...
            if (columns.length !== expectedColumns.length || columns.some((row, index) => row.name !== expectedColumns[index] || row.hidden !== 0) || rowArray(this.storage, `PRAGMA foreign_key_list(${table})`).length !== 0)
                throw new IncompatibleSqliteSchema("application columns rejected");
            const indexes = rowArray(this.storage, `PRAGMA index_list(${table})`);
            if (indexes.length !== 1 || indexes[0]?.name !== `sqlite_autoindex_${table}_1` || indexes[0]?.unique !== 1 || indexes[0]?.origin !== "pk" || indexes[0]?.partial !== 0)
                throw new IncompatibleSqliteSchema("application index rejected");
        }
        ...
        if (JSON.stringify(values) !== JSON.stringify(wantedSorted))
            throw new IncompatibleSqliteSchema("application metadata rejected");
    }
    async transact(work) { return this.storage.transactionSync(() => work(new SqliteTransaction(this.storage))); }
    async read(key) { const row = rowArray(this.storage, "SELECT value_json FROM kv WHERE key=?", key)[0]; return row === undefined ? undefined : JSON.parse(row.value_json); }
    entries(prefix) { if (!/^(receipt|generation|outbox|push-child|binding|current)\/$/.test(prefix))
        throw new Error("scheduler prefix rejected"); return rowArray(this.storage, "SELECT key,value_json FROM kv WHERE key LIKE ? ORDER BY key", `${prefix}%`).map((row) => ({ key: row.key, value: JSON.parse(row.value_json) })); }
    rowCount() { return Number(rowArray(this.storage, "SELECT count(*) AS count FROM kv")[0].count); }
    ...
    reserveQuota(now, vector) { return reserveQuota(this.storage, now, vector); }
    readMeta(key) { const row = rowArray(this.storage, "SELECT value_json FROM meta WHERE key=?", key)[0]; return row === undefined ? undefined : JSON.parse(row.value_json); }
...
class SqliteTransaction {
    ...
    get(key) { const row = rowArray(this.storage, "SELECT value_json FROM kv WHERE key=?", key)[0]; return row === undefined ? undefined : JSON.parse(row.value_json); }
    putIfAbsent(key, value) { const row = this.storage.sql.exec("INSERT INTO kv(key,version,value_json) VALUES(?,1,?) ON CONFLICT(key) DO NOTHING RETURNING key", key, JSON.stringify(value)).toArray()[0]; if (row === undefined)
