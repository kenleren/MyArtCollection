...
const positive = (value) => typeof value === "number" ? Number.isSafeInteger(value) && value > 0 : /^[0-9a-f]{40}$/.test(value);
function requireIdentity(identity, route, args) {
    if (!Number.isSafeInteger(identity.installationId) || identity.installationId <= 0 || identity.repositoryId !== REPOSITORY_ID || args.some((arg) => !positive(arg)))
        throw new Error("github route rejected");
    if (route === "installationToken") {
        if (args.length !== 1 || args[0] !== identity.installationId)
            throw new Error("github route rejected");
        return;
    }
    if (args[0] !== identity.repositoryId)
        throw new Error("github route rejected");
}
const routes = {
...
export function githubRoute(identity, route, args, query = "") {
...
export function isExactGithubRequest(identity, request) {
    const url = new URL(request.url);
    if (url.origin !== githubApiOrigin || url.username || url.password || url.hash || request.redirect !== "manual" || request.headers.get("accept") !== "application/vnd.github+json" || request.headers.get("x-github-api-version") !== GITHUB_API_VERSION)
        return false;
