...
test("one alarm memoizes one token and rejects subrequest 51 before send", async () => {
  ...
  const fetcher: typeof fetch = async (input) => {
    ...
    if (request.url.endsWith("/app/installations/2/access_tokens")) { tokenCalls += 1; assert.equal(request.headers.get("content-type"), "application/json"); assert.equal(await request.text(), '{"repository_ids":[1288597824],"permissions":{"checks":"write","contents":"read","metadata":"read","pull_requests":"read"}}'); const expiry = new Date((Math.floor(Date.now() / 1000) + 120) * 1000).toISOString().replace(".000", ""); return new Response(JSON.stringify({ token: "synthetic", expires_at: expiry, permissions: { checks: "write", contents: "read", metadata: "read", pull_requests: "read" }, repository_selection: "selected", repositories: [{ id: 1288597824, name: "MyArtCollection", full_name: "kenleren/MyArtCollection" }] }), { status: 201, headers: { "content-type": "application/json" } }); }
