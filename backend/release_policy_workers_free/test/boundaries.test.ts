...
import worker from "../src/worker.js"; import { classifyGitHubEgress, enforceGitHubEgress, GitHubAppPort, githubAlarmPort } from "../src/github_app_port.js";
import { boundedBucket } from "../src/telemetry.js";
const frozen = { installationId: 2, repositoryId: 1288597824 as const };
...
test("one alarm memoizes one token and rejects subrequest 51 before send", async () => {
  ...
  const fetcher: typeof fetch = async (input) => {
    ...
    if (request.url.endsWith("/app/installations/2/access_tokens")) { tokenCalls += 1; assert.equal(request.headers.get("content-type"), "application/json"); assert.equal(await request.text(), '{"repository_ids":[1288597824],"permissions":{"checks":"write","contents":"read","metadata":"read","pull_requests":"read"}}'); const expiry = new Date((Math.floor(Date.now() / 1000) + 120) * 1000).toISOString().replace(".000", ""); return new Response(JSON.stringify({ token: "synthetic", expires_at: expiry, permissions: { pull_requests: "read", metadata: "read", checks: "write", contents: "read" }, repository_selection: "selected", repositories: [{ id: 1288597824, node_id: "R_test", name: "MyArtCollection", full_name: "kenleren/MyArtCollection", private: true, owner: { login: "kenleren" } }], expires_at_extra: "documented fields permitted" }), { status: 201, headers: { "content-type": "application/json" } }); }
  ...
  const measurements: Array<{ metric: string; value: number }> = [];
  ...
  assert.equal(calls, 50); assert.equal(tokenCalls, 1);
  assert.equal(measurements.filter((row) => row.metric === "request_high_water").at(-1)?.value, 51); assert.equal(measurements.filter((row) => row.metric === "exceeded_resource").length, 1);
});
