...
test("Link absence terminates even a full final page", async () => {
  ...
  assert.equal(page.items.length, 100); assert.equal(page.nextPage, null);
});
...
test("pagination rejects page 31 before response body consumption", async () => {
  ...
  const response = { redirected: false, ok: true, headers: new Headers({ "content-type": "application/json", link }), get body() { reads++; return new ReadableStream<Uint8Array>(); } } as unknown as Response;
...
test("every malformed or contradictory Link rejects before body consumption", async () => {
  ...
  for (const [index, link] of cases.entries()) {
    let bodyReads = 0;
    const response = { redirected: false, ok: true, headers: new Headers({ "content-type": "application/json", link }), get body() { bodyReads += 1; return new ReadableStream<Uint8Array>({ start(controller) { controller.enqueue(new TextEncoder().encode("[]")); controller.close(); } }); } } as unknown as Response;
