...
const value = (name: string): string => { const i = process.argv.indexOf(`--${name}`); const result = i < 0 ? undefined : process.argv[i + 1]; if (!result || result.startsWith("--")) throw new Error(`SBOM ${name} required`); return result; };
const root = resolve(process.cwd()); const repositoryRoot = execFileSync("git", ["-C", root, "rev-parse", "--show-toplevel"], { encoding: "utf8" }).trim(); const anchoredPath = (input: string) => input.startsWith("backend/release_policy_workers_free/") ? resolve(repositoryRoot, input) : resolve(input);
const anchor = value("anchor"); const output = anchoredPath(value("output"));
const lockInput = process.argv.includes("--lock") ? anchoredPath(value("lock")) : resolve(root, "package-lock.json");
if (!/^[0-9a-f]{40}$/.test(anchor) || !output || output === root || !lockInput) throw new Error("SBOM anchor rejected");
const git = (...args: string[]) => execFileSync("git", ["-C", root, ...args], { encoding: "utf8" }).trim();
const epoch = Number(git("show", "-s", "--format=%ct", anchor)); if (!Number.isSafeInteger(epoch)) throw new Error("SBOM epoch rejected");
...
if (process.argv.includes("--lock") && !readFileSync(lockInput).equals(lockBytes)) throw new Error("SBOM lock differs from source anchor");
const lock = JSON.parse(lockBytes.toString("utf8")) as { name: string; version: string; packages: Record<string, { name?: string; version?: string; integrity?: string }> };
...
const rootPackage = packages[0]; if (!rootPackage) throw new Error("SBOM root package missing");
...
mkdirSync(dirname(output), { recursive: true }); writeFileSync(output, `${JSON.stringify(document)}\n`, { flag: "w" });
