...
const gitPaths = Object.fromEntries(files.map((path) => {
  const absolute = resolve(root, path); const gitPath = relative(gitRoot, absolute);
  if (!gitPath || gitPath === ".." || gitPath.startsWith(`..${sep}`)) throw new Error(`artifact path outside repository: ${path}`);
  execFileSync("git", ["-C", gitRoot, "ls-files", "--error-unmatch", "--", gitPath], { stdio: "ignore" });
  return [path, gitPath];
}));
const gitPathFor = (path: string): string => { const value = gitPaths[path]; if (!value) throw new Error(`artifact git path unavailable: ${path}`); return value; };
const hashesAt = (sha: string): Record<string, string> => Object.fromEntries(files.map((path) => [path, sha256(execFileSync("git", ["-C", gitRoot, "show", `${sha}:${gitPathFor(path)}`]))]));
if (process.argv[2] === "generate") {
  const dirty = files.filter((path) => {
    try { execFileSync("git", ["-C", gitRoot, "diff", "--quiet", currentSha, "--", gitPathFor(path)], { stdio: "ignore" }); return false; } catch { return true; }
  });
  if (dirty.length > 0) throw new Error(`artifact files must be committed before anchoring: ${dirty.join(",")}`);
  const anchorDrift = artifactDrift(fileHashes, fileHashes, hashesAt(currentSha));
  if (anchorDrift.length > 0) throw new Error(`artifact anchor tree drift: ${anchorDrift.join(",")}`);
