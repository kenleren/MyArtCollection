...
if (process.argv[2] === "generate") writeFileSync(target, canonicalManifest(fileHashes, currentSha));
  ...
  if (!recorded.git_sha || !/^[0-9a-f]{40}$/.test(recorded.git_sha) || JSON.stringify(recorded.files) !== JSON.stringify(Object.fromEntries(Object.entries(fileHashes).sort(([a], [b]) => a.localeCompare(b))))) throw new Error("artifact manifest drift");
