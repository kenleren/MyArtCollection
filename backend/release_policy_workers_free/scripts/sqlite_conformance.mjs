...
function boundedEvidence(results) {
  return { schema_version: 1, node: process.version, miniflare: "4.20260714.0", bundle_sha256: bundleHash, import_manifest_sha256: importManifestHash, paired_black_box: true, paired_persistent_sqlite: true, unsafe_live_inspection: false, scheduled_proxy: true, removed_miniflare_dispatch_absent: true, scheduled_http_trigger_count: 0, case_count: results.length, cases: results.map((result) => ({ case_id: result.caseId, external_equivalence: true, route_count: result.external.totals.all, create_count: result.external.totals.create, update_count: result.external.totals.update, forbidden_egress_count: result.external.forbidden_egress, row_count: Math.max(0, ...result.inspections.map((row) => row.rows)), database_bytes: Math.max(0, ...result.inspections.map((row) => row.database_bytes)), alarm_count: Math.max(0, ...result.inspections.map((row) => row.alarm_count)), redaction_scan: result.inspections.every((row) => row.redaction_scan) })), temp_cleanup: true, production_config_leakage: false, deployed_cpu_quota_proof: false };
}
