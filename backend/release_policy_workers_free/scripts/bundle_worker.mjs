...
const outdir = resolve(root, "evidence/bundle");
const runtimeOutdir = resolve(root, "../../.work/release-policy-workers-free/bundle");
...
writeFileSync(resolve(outdir, "bundle-evidence.v1.json"), JSON.stringify({ sha256: createHash("sha256").update(output).digest("hex"), bytes: output.byteLength, esbuild: version, miniflare: "4.20260721.0", compatibility_flags: ["nodejs_compat", "nodejs_compat_v2", "nodejs_compat_do_not_populate_process_env", "disallow_importable_env"] }) + "\n");
for (const name of ["worker.mjs", "metafile.json", "import-manifest.v1.json", "bundle-evidence.v1.json"]) copyFileSync(resolve(outdir, name), resolve(runtimeOutdir, name));
