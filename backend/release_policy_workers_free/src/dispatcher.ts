import {
  ...
  prepareCheckBinding, receive, runCreateCheck, reconcileCreateCheckOnce, runUpdateCheck, settlePullReceipt,
...
export class AlarmDispatcher {
  ...
  async alarm(): Promise<void> {
    const quota = typeof (this.store as unknown as { reserveQuota?: unknown }).reserveQuota === "function" ? this.store.reserveQuota(Date.now(), { alarms: 1 }) : { admitted: true, rolloverAt: Date.now() + 25 };
    if (!quota.admitted) { await this.storage.setAlarm(quota.rolloverAt); return; }
    ...
    try {
      ...
      for (const row of this.store.entries("receipt/")) {
        ...
        const retry = this.store.readMeta<{ retry_at?: number; state?: string }>(`retry/${receipt.installationId}/${receipt.deliveryId}`);
        if (retry?.state === "quarantined") continue;
        if (typeof retry?.retry_at === "number" && retry.retry_at > Date.now()) { retryAt = Math.min(retryAt ?? Infinity, retry.retry_at); continue; }
  ...
  private async advanceGeneration(generationId: string, port: GitHubCheckRunsPort): Promise<void> {
    ...
    if (afterBinding?.state === "decision_ready") {
      const reconcileKey = `reconcile/${generationId}/v1`; const reconciliation = this.store.readMeta<{ attempt: number; due_at: number; cleared?: boolean }>(reconcileKey);
      if (reconciliation && !reconciliation.cleared) {
        ...
        if (result === "not_visible") { const delay = [1, 2, 4, 8, 16, 32][reconciliation.attempt + 1] ?? 32; this.store.writeMeta(reconcileKey, { attempt: reconciliation.attempt + 1, due_at: Date.now() + delay * 1000 }); }
        else this.store.writeMeta(reconcileKey, { attempt: reconciliation.attempt, due_at: 0, cleared: true });
      } else {
        try { await runCreateCheck({ clock: this.runtime.clock, generationId, port, store: this.store, worker }); }
        catch (error) { if (error instanceof AmbiguousCreateError) { this.store.writeMeta(reconcileKey, { attempt: 0, due_at: Date.now() + 1000 }); return; } throw error; }
      }
    }
  ...
  private hasPendingWork(): boolean { return this.store.entries("receipt/").some(({ value }) => { const receipt = value as Receipt; const retry = this.store.readMeta<{ state?: string }>(`retry/${receipt.installationId}/${receipt.deliveryId}`); return !["terminal_success", "terminal_failure", "conflict"].includes(receipt.state) && retry?.state !== "quarantined"; }); }
